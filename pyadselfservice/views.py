...
def ADValidate(request):
     ...
     if request.method == 'POST':
        ...
        if form.is_valid():
           output = do_validate(form.cleaned_data['username'], form.cleaned_data['attr3'])
           if output == 'YGFRafd827343wdhgFDHGFDSFGHVFSNC':
             cipher_text = form.cleaned_data['username']
             data = signer.sign(cipher_text)
             return HttpResponseRedirect('/otp?key=' + quote(data))
           else:
             value = str(output)
             return HttpResponseServerError(template.render({'exception_value': value,}))
      ...
      if form.is_valid():
         ...
         if newpassword and newpassword != confirmpassword:
            ...
            if 'success' in output:
               ...
               return HttpResponseServerError(loader.get_template('success.html').render({'exception_value': value,}))
            else:
               value = str(output)
               return HttpResponseServerError(template.render({'exception_value': value,}))
      else:
